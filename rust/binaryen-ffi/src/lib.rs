...
/// # Safety
/// Caller must ensure `s` is either null or points to a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn binaryen_ffi_echo(s: *const c_char) -> *const c_char {
    if s.is_null() {
        return std::ptr::null();
    }
...
/// # Safety
/// Caller must ensure `p` points to a valid `BinaryenStringInterner` and `s` is either null or points to a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn BinaryenStringInternerIntern(
...
fn handle_to_arena_ptr(h: *mut BinaryenArenaHandle) -> Option<*mut binaryen_support::Arena> {
    ...
    unsafe {
        ...
        map.get(&id)
            .map(|ptr| (*ptr) as *mut binaryen_support::Arena)
    }
}

/// # Safety
/// Caller must ensure `h` points to a valid `BinaryenArenaHandle` and `s` is either null or points to a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn BinaryenArenaHandleAllocString(
...
/// # Safety
/// Caller must ensure `p` points to a valid `BinaryenArena` and `s` is either null or points to a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn BinaryenArenaAllocString(
    p: *mut BinaryenArena,
    s: *const c_char,
) -> *const c_char {
    if p.is_null() || s.is_null() {
...
///
/// # Safety
/// Caller must ensure `data` is either null or points to a valid buffer of at least `len` bytes.
#[no_mangle]
pub unsafe extern "C" fn BinaryenAhashBytes(data: *const c_uchar, len: usize) -> u64 {
    if data.is_null() {
        return 0;
    }
    let slice = std::slice::from_raw_parts(data, len);
    binaryen_support::hash::ahash_bytes(slice)
}
...
pub extern "C" fn BinaryenFastHashMapCreate() -> *mut BinaryenFastHashMap {
    let m: Box<binaryen_support::hash::FastHashMap<String, u64>> = Box::default();
...
/// # Safety
/// Caller must ensure `p` points to a valid `BinaryenFastHashMap` and `key` is either null or points to a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn BinaryenFastHashMapInsert(
...
/// # Safety
/// Caller must ensure `p` points to a valid `BinaryenFastHashMap`, `key` is either null or points to a valid null-terminated C string, and `out_value` is either null or points to a valid `u64`.
#[no_mangle]
pub unsafe extern "C" fn BinaryenFastHashMapGet(
    p: *mut BinaryenFastHashMap,
    key: *const c_char,
    out_value: *mut u64,
) -> bool {
...
mod tests {
    ...
    fn test_ffi_echo_and_null() {
        unsafe {
            ...
            assert!(binaryen_ffi_echo(std::ptr::null()).is_null());
        }
    }
