...
- If you intend to change the ABI (shape, names, types or ownership semantics), do the following:
    1. Update the rust code in `rust/binaryen-ffi` and rerun `scripts/check_cbindgen.sh` locally.  
...
- To change the ABI, bump the constant in Rust (`rust/binaryen-ffi/src/lib.rs`) and regenerate header with `scripts/update_cbindgen.sh`.
...
3. Run `scripts/update_cbindgen.sh` and review the resulting `include/binaryen_ffi.h` to ensure the `BINARYEN_FFI_ABI_VERSION` symbol is present in the form you want.
...
- Sign-off checklist for ABI changes:
    ...
    3. Run `scripts/check_cbindgen.sh` locally, and include the command output in the PR if the header changed.  
