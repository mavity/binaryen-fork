...
**Status**: 📋 Planned | **Passes**: 24 | **Estimated Effort**: 3–4 weeks | **Target**: Week 3-5
...
**Critical Prerequisite**: ⚠️ **Arena-based expression manipulation** must be implemented first to enable tree restructuring. See "Critical Architectural Task" section in main gap analysis.
...
### Pass List
1. **simplify-locals** (⚠️ partially done) — Sink local.sets, create tees, optimize structure
2. **coalesce-locals** (✅ done) — Merge non-overlapping locals
3. **local-cse** — Local common subexpression elimination
4. **local-subtyping** — Refine local types to more specific subtypes
5. **untee** — Convert local.tees back to sets+gets
6. **merge-locals** — Merge similar local patterns
7. **dae** (dead-argument-elimination) — Remove unused function parameters
8. **dae-optimizing** — DAE + optimize where we removed args

### Shared Infrastructure

**LocalGraph** (2-3 days, reusable):
```rust
// rust/binaryen-ir/src/dataflow/local_graph.rs
...
```rust
// rust/binaryen-ir/src/analysis/dominators.rs
...
**What's Done**:
- ✅ Pass infrastructure (3 modes: allow_tee, allow_structure, allow_nesting)
- ✅ FunctionContext tracking sinkables and use counts
...
### Pass List
1. **merge-blocks** — Combine sequential blocks
2. **simplify-control-flow** — Flatten nested structures, remove empty blocks
3. **rereloop** — Convert irreducible CFG back to structured form
4. **poppify** — Optimize pop/drop patterns in stack machine code
5. **rse** (redundant-set-elimination) — Remove redundant local.sets
6. **flatten** — Convert nested blocks to flat IR (needed for DataFlowOpts)
7. **code-pushing** — Move code closer to uses without crossing barriers
8. **licm** (loop-invariant-code-motion) — Hoist invariants out of loops
...
**Patterns**:
- Empty `if` with no else → remove
- `if` with identical then/else → extract common code
- Nested blocks with no intermediate code → flatten
...
### Pass List
1. **pick-load-signs** — Infer optimal signedness for loads
2. **signext-lowering** — Lower sign-extension operations to MVP
3. **avoid-reinterprets** — Prevent unsafe type punning
4. **optimize-added-constants** / **optimize-added-constants-propagate** — Fold constant offsets into loads/stores
5. **ssa** / **ssa-nomerge** — Convert to SSA form
6. **instrument-locals** — Add instrumentation for debugging
7. **trap-mode-clamp** / **trap-mode-js** — Configurable trap behavior
...
### Quantitative
- ✅ 24 passes implemented (6 → 30 total)
- ✅ Functional parity: ~40% of C++ capability
- ✅ Code size reduction: 15-25% on typical Emscripten output
- ✅ All 300+ existing tests pass
- ✅ 150+ new tests added (6-8 per pass)

### Qualitative
- ✅ Single-function optimization pipeline complete
- ✅ Reusable CFG and dominance infrastructure in place
- ✅ Arena-based tree manipulation proven (unblocks Tier 3+)
...
- ✅ Single-function optimizations work end-to-end
- ✅ All foundational analyses implemented
- ✅ Arena-based tree manipulation proven
...
| :--- | :--- | :--- | :--- |
| `simplify_locals.rs` | [SimplifyLocals.cpp](../../../src/passes/SimplifyLocals.cpp) | **100%** | **Complete**: Implements CFG-aware sinking, aggressive merging, and structural optimizations (block, if, loop). |
| `coalesce_locals.rs` | [CoalesceLocals.cpp](../../../src/passes/CoalesceLocals.cpp) | **80%** | Implements basic interference coloring and copy coalescing; missing multi-value locals. |
...
| `merge_blocks.rs` | [MergeBlocks.cpp](../../../src/passes/MergeBlocks.cpp) | **30%** | Handles basic block flattening and blockifying; missing sequential block-chaining. |
| `local_subtyping.rs` | [LocalSubtyping.cpp](../../../src/passes/LocalSubtyping.cpp) | **30%** | **Shell**: Basic logic flow exists but lacks robust Least Upper Bound (LUB) calc and GC type support. |
| `simplify_control_flow.rs` | [SimplifyControlFlow.cpp](../../../src/passes/SimplifyControlFlow.cpp) | **5%** | **Stub**: Identity transformation only; missing pattern matching for if/else folding. |
...
| `optimize_added_constants.rs` | [OptimizeAddedConstants.cpp](../../../src/passes/OptimizeAddedConstants.cpp) | **5%** | **Stub**: Missing offset merging logic. |
| `code_pushing.rs` | [CodePushing.cpp](../../../src/passes/CodePushing.cpp) | **5%** | **Stub**: No logic for safe code motion across blocks. |
| `poppify.rs` | [Poppify.cpp](../../../src/passes/Poppify.cpp) | **5%** | **Stub**: Infrastructure exists but doesn't perform stack optimizations. |
| `signext_lowering.rs` | [SignextLowering.cpp](../../../src/passes/SignextLowering.cpp) | **5%** | **Stub**: No lowering logic for sign-extension ops. |
| `avoid_reinterprets.rs` | [AvoidReinterprets.cpp](../../../src/passes/AvoidReinterprets.cpp) | **5%** | **Stub**: No analysis of reinterpretation instructions. |
| `instrument_locals.rs` | [InstrumentLocals.cpp](../../../src/passes/InstrumentLocals.cpp) | **5%** | **Stub**: Missing instrumentation insertion logic. |

## Remaining Work Summary

To achieve full Tier 2 parity, the following technical blocks must be addressed:

1.  **Core Algorithmic Implementation** (High Effort):
    *   **SSA Construction**: Implement the Cytron et al. algorithm or similar for `ssa.rs`, including Dominance Frontier calculation and Phi-node insertion.
    *   **Relooper/Rereloop**: Port the complex logic from `src/cfg/Relooper.cpp` to `rereloop.rs` to handle irreducible control flow.
    *   **Loop Analysis**: Enhance `cfg.rs` to support natural loop detection and nesting depth calculation for `licm.rs`.

2.  **Pattern Matching & Instruction Optimization**:
    *   **SimplifyControlFlow**: Implement the ~20+ specific patterns for folding `if` branches and flattening blocks.
    *   **OptimizeAddedConstants**: Write the recursive expression walker to propagate and fold constant offsets into loads/stores.

3.  **Data Flow Depth**:
    *   **LocalCSE**: Implement the recursive replacement logic and value-numbering cache.
    *   **LocalSubtyping**: Bridge the `binaryen-core` type system to allow robust Least Upper Bound (LUB) calculations, especially for GC-enabled modules.

4.  **Edge Case Parity**:
    *   **CoalesceLocals**: Add support for multi-value locals and more aggressive interference graph coloring.
    *   **MergeBlocks**: Implement multi-block sequential merging (currently only handles 1-to-1 parent/child nesting).
