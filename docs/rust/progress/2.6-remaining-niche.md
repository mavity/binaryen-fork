...
**Status**: 📋 Planned (Low Priority) | **Passes**: 21 | **Estimated Effort**: Variable (as-needed) | **Target**: On-demand
...
### Pass List
1. **catch-pop-fixup** — Fix nested pops within catch blocks (EH testing)
2. **deinstrument-branch-hints** — Remove branch hint instrumentation
3. **delete-branch-hints** — Delete branch hints using list of instrumented IDs
4. **experimental-type-generalizing** — Generalize types (⚠️ not yet sound, testing only)
5. **randomize-branch-hints** — Randomize branch hints for fuzzing
6. **reorder-globals-always** — Force global reordering even with few globals
7. **reorder-types-for-testing** — Use exaggerated cost function for reordering
...
### Pass List
1. **extract-function** — Leave just one function (useful for debugging large modules)
2. **extract-function-index** — Leave just one function selected by index
3. **roundtrip** — Write module to binary, read it back (validation utility)
4. **func-metrics** — Report function metrics (size, complexity, etc.)
...
**Priority Order**:
1. **roundtrip** (1-2 days) — Highest value, validates binary I/O
2. **func-metrics** (1 day) — Useful for profiling and optimization
3. **extract-function** (1 day) — Helpful for debugging
4. **extract-function-index** (0.5 day) — Variant of extract-function
...
### Pass List
1. **dealign** — Force all loads/stores to alignment=1 (testing)
2. **separate-data-segments** — Write data segments to separate file
3. **limit-segments** — Merge segments to fit web loader limits
...
### Pass List
1. **once-reduction** — Reduce calls to code that only runs once
2. **enclose-world** — Destructive closed-world modifications
3. **set-globals** — Set specified globals to values (testing/specialized)
4. **duplicate-import-elimination** — Remove duplicate imports
...
**Priority**:
1. **duplicate-import-elimination** (1 day) — Can be included in Tier 1 if time permits
2. **once-reduction** (2-3 days) — If optimizing startup code becomes priority
3. **set-globals** (0.5 day) — Only if testing infrastructure needs it
4. **enclose-world** (3-4 days) — Only for highly specialized use cases
...
### Pass List
1. **instrument-branch-hints** — Instrument branch hints to track correctness
2. **remove-non-js-ops** — Remove operations incompatible with JS
3. **stub-unsupported-js** — Stub out unsupported JS operations
...
1. **roundtrip** — Validates binary I/O correctness
2. **func-metrics** — Profiling aid for optimization
3. **extract-function** — Debugging large modules
4. **duplicate-import-elimination** — Small code size win
5. **once-reduction** — Niche but legitimate optimization
...
### Precipitous Summary
The "Remaining Passes" (Group R1-R5) represent the tail end of the Binaryen port. Currently, **0 out of 21** passes in this category have been ported. This is a deliberate prioritization choice: these passes are either testing utilities, niche optimizations, or instrumentation tools not required for the core "Functional Parity" goal.

**Conclusion**: This tier exists as a placeholder for ecosystem completeness. Until the core optimization tiers (1-4) reach 100% maturity, the maturity of this tier will remain at 0%.

### Maturity Matrix

| Pass | Rust File | C++ File (Ref) | Maturity % | Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Catch-Pop-Fixup** | N/A | [CatchPopFixup.cpp](../../../src/passes/CatchPopFixup.cpp) | 0% | **Missing** | Critical for complex EH testing. |
| **Extract Function** | N/A | [ExtractFunction.cpp](../../../src/passes/ExtractFunction.cpp) | 0% | **Missing** | High-value debugging utility. |
| **RoundTrip** | N/A | [RoundTrip.cpp](../../../src/passes/RoundTrip.cpp) | 0% | **Missing** | Essential for Binary I/O validation. |
| **Metrics** | N/A | [Metrics.cpp](../../../src/passes/Metrics.cpp) | 0% | **Missing** | Diagnostic tool for module stats. |
| **Duplicate Import Elim**| N/A | [DuplicateImportElimination.cpp](../../../src/passes/DuplicateImportElimination.cpp) | 0% | **Missing** | Small code size win; low porting cost. |
| **Once Reduction** | N/A | [OnceReduction.cpp](../../../src/passes/OnceReduction.cpp) | 0% | **Missing** | Niche optimization for startup code. |
| **DeAlign** | N/A | [DeAlign.cpp](../../../src/passes/DeAlign.cpp) | 0% | **Missing** | Stress-testing for alignment issues. |
| **Limit Segments** | N/A | [LimitSegments.cpp](../../../src/passes/LimitSegments.cpp) | 0% | **Missing** | Browser-compat utility. |
| **Enclose World** | N/A | [EncloseWorld.cpp](../../../src/passes/EncloseWorld.cpp) | 0% | **Missing** | Aggressive, destructive whole-module optimization. |
...
- `2-gap-analysis.md` — Overall strategy and timeline
- `2.1-tier1-foundational.md` through `2.5-tier5-specialized.md` — Core implementation tiers
- `README-PASSES.md` — Navigation guide
