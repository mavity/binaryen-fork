...
| `remove_memory_init.rs` | [RemoveMemoryInit.cpp](../../../src/passes/RemoveMemoryInit.cpp) | **80%** | Removes data segments, but only unsets `start` field instead of removing the function itself to avoid index shifts. |
...
| `simplify_identity.rs` | [SimplifyIdentity.cpp](../../../src/passes/SimplifyIdentity.cpp) | **70%** | Basic arithmetic identities implemented; lacks redundant cast removal and more advanced bitwise rules. |
| `reorder-* / minify-*` | [ReorderTypes.cpp](../../../src/passes/ReorderTypes.cpp) | **15%** | Reorder files exist but are logic-dry stubs; `ModuleStats` doesn't track types; `minify` passes are completely missing. |
| `strip-* / print-*` | [StripDebug.cpp](../../../src/passes/StripDebug.cpp) | **5%** | `strip-debug` is a skeletal stub that only clears annotations; all `print-*` passes are missing. |
