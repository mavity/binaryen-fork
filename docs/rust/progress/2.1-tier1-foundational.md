...
Tier 1 is now over 90% complete. Group 1B (Reordering) is fully functional with usage-based sorting for all major module elements. Group 1C (Metadata/Printing) has seen progress with WAT printing and name minification, while Group 1A has been expanded to include comprehensive type pruning.
...
| `simplify_identity.rs` | [SimplifyIdentity.cpp](../../../src/passes/SimplifyIdentity.cpp) | **95%** | Extensive identities for arithmetic, bitwise ops (x&0, x|-1, etc.), and block pull-up. Missing redundant cast removal. |
...
| `minify-names / strip-names` | [MinifyImports.cpp](../../../src/passes/MinifyImports.cpp) | **70%** | Functional name minification for functions; lacks dedicated import/export minification and global name shortening. |
| `strip-* / print-*` | [StripDebug.cpp](../../../src/passes/StripDebug.cpp) | **50%** | `Print` pass provides WAT output; `StripDebug` remains a basic annotation-clearing stub; missing specialized printers. |
