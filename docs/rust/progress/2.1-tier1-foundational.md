...
**Status**: 📋 Planned | **Passes**: 19 | **Estimated Effort**: 2–3 weeks | **Target**: Week 1-2
...
**Key Principle**: These passes are **parallelizable** — multiple team members can implement different groups simultaneously after shared infrastructure is in place.
...
### Pass List
1. **remove-unused-module-elements** — Remove unused functions, globals, data segments
2. **remove-unused-brs** — Eliminate branches to labels that are never used
3. **remove-unused-names** — Strip debug names from locations never branched to
4. **remove-unused-types** — Eliminate unreachable type definitions (GC types)
5. **remove-imports** — Strip unused imports (configurable)
6. **remove-memory-init** / **remove-memory** — Remove data segment initialization
7. **vacuum** — Bulk removal of obviously unneeded code
...
### Success Criteria
- ✅ All 7 passes pass unit tests
- ✅ UsageTracker has 100% code coverage
- ✅ No regressions in existing 300+ tests
- ✅ Documentation: Each pass has docstring explaining what it removes
...
### Pass List
1. **reorder-types** — Sort types by usage frequency for better compression
2. **reorder-locals** — Sort locals to improve LEB128 encoding
3. **reorder-globals** — Reorder globals by access frequency
4. **reorder-functions** / **reorder-functions-by-name** — Sort functions for locality/debugging
5. **minify-imports** / **minify-imports-and-exports** — Shorten import/export names
...
### Success Criteria
- ✅ All 5 passes correctly reorder elements
- ✅ ModuleStats accurately counts references
- ✅ Validation: Module validates before and after reordering
- ✅ Binary size test: Reordered module compresses better (measure with gzip)
...
### Pass List
1. **name-types** — Assign stable names to heap types
2. **nm** (name-list) — Generate name list for debugging
3. **propagate-debug-locs** — Propagate DWARF location info through tree
4. **strip-debug** — Remove debug info including names section
5. **strip-dwarf** — Remove DWARF sections specifically
6. **emit-target-features** — Write target features section
7. **print** / **print-minified** / **print-full** / **print-call-graph** / **print-function-map** — Various output formats
...
### Success Criteria
- ✅ Strip passes correctly remove targeted metadata
- ✅ Print passes produce valid output matching C++ format
- ✅ Propagate-debug-locs correctly inherits location info
- ✅ No semantic changes to module execution
...
### Testing
- [ ] Each pass has ≥3 unit tests (basic, edge case, preservation)
...
- ✅ Clean IR (unused elements removed)
- ✅ Optimized layout (reordered for compression)
...
While the plumbing for Tier 1 cleanup is partially established, there is a severe disparity between the planned list and the functional maturity of the code. Group 1A (Dead Code Removal) contains legitimate logic but simplifies the problem by ignoring imports and table elements. Entire domains—specifically Group 1B (Reordering for compression) and Group 1C (Metadata management)—are completely missing from the codebase, existing only as placeholders in documentation.

### Tier 1 Pass Maturity Matrix

| Rust Pass File | Equivalent C++ Implementation | Port % | Critical Gap / Missing Piece |
| :--- | :--- | :--- | :--- |
| `remove_unused_module_elements.rs` | [RemoveUnusedModuleElements.cpp](../../../src/passes/RemoveUnusedModuleElements.cpp) | **90%** | Utilizes `UsageTracker` to recursively prune functions, globals, and imports. Implements full index remapping across module and exports. |
| `remove_unused_brs.rs` | [RemoveUnusedBrs.cpp](../../../src/passes/RemoveUnusedBrs.cpp) | **80%** | Implements robust flow-based optimization (`Flow` analysis). Handles redundant breaks, unreachable code, and block fallthrough simplification. |
| `remove_unused_names.rs` | [RemoveUnusedNames.cpp](../../../src/passes/RemoveUnusedNames.cpp) | **70%** | Analyzes branch targets to prune unused labels from blocks, loops, and switch expressions. |
| `remove_imports.rs` | [RemoveImports.cpp](../../../src/passes/RemoveImports.cpp) | **80%** | Tracks import usage and replaces unnecessary calls with side-effect-aware placeholders (via `CallReplacer`). |
| `remove_memory_init.rs` | [RemoveMemoryInit.cpp](../../../src/passes/RemoveMemoryInit.cpp) | **30%** | **Basic**: Clears data segments and start functions; missing the fine-grained redundancy analysis of the C++ original. |
| `vacuum.rs` | [Vacuum.cpp](../../../src/passes/Vacuum.cpp) | **90%** | Fully recursive simplification using `EffectAnalyzer`. Handles block reduction, `if`-condition folding, and dead-expression pruning. |
| `simplify_identity.rs` | [SimplifyIdentity.cpp](../../../src/passes/SimplifyIdentity.cpp) | **85%** | Extensive identities for integer arithmetic (x+0, x*1), bitwise ops (x&~0), and double reference type casting. |
| `reorder-* / minify-*` | [ReorderTypes.cpp](../../../src/passes/ReorderTypes.cpp) | **0%** | **Missing**: No files exist for Group 1B (Reordering & Reorganization). |
| `strip-* / print-*` | [StripDebug.cpp](../../../src/passes/StripDebug.cpp) | **0%** | **Missing**: No passes exist for Group 1C (Metadata & Name Handling). |
