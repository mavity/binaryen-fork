...
- [x] Implement a nightly Criterion benchmarks job that uploads artifacts (comparator TBD) — nightly job uploads artifacts; comparator logic is a follow-up item.
- [x] Expand `miri` and sanitizer coverage (ASAN/UBSAN) for critical crates and add scheduled checks in CI (Miri now runs for `binaryen-support` and `binaryen-core` at nightly schedule; sanitizer expansion is pending).
- [x] Add a small set of cross-language C++ integration tests beyond the smoke consumer to better validate ownership and memory contracts across FFI boundaries (added `test_ffi_extra.c` and CI `ctest` test `rust_ffi_smoke_extra`).
