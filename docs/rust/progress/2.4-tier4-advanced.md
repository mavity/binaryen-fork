...
### Precipitous Summary
Tier 4 represents the bridge between a "functional" port and a "compiling" port. While the **infrastructure** (CallGraph, CFG, SSA) is largely present and used by the skeletal passes, the **optimization logic** is heavily trailing. Major passes like `Inlining` and `DataFlowOpts` (DFO) are currently "logic-dry"—they correctly traverse the IR and build analysis structures but lack the hundreds of specific optimization cases present in the C++ original.

**Conclusion**: The "Bones" of Tier 4 are in place, but the "Muscle" (rulesets and heuristics) is missing for ~75% of the advanced transforms.

### Maturity Matrix

| Pass | Rust File | C++ File (Ref) | Maturity % | Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Inlining** | `inlining.rs` | [Inlining.cpp](../../../src/passes/Inlining.cpp) | 15% | **Partial** | Basic trivial-call inlining. Lacks production cost models. |
| **DFO (DataFlowOpts)** | `dataflow_opts.rs` | [DataFlowOpts.cpp](../../../src/passes/DataFlowOpts.cpp) | 10% | **Skeletal** | CFG/SSA boilerplate exists; no optimization logic (DCE/Prop). |
| **Flatten** | `flatten.rs` | [Flatten.cpp](../../../src/passes/Flatten.cpp) | 5% | **Skeletal** | No logic to convert to flat IR. |
| **Duplicate Func Elim**| `duplicate_function_elimination.rs` | [DuplicateFunctionElimination.cpp](../../../src/passes/DuplicateFunctionElimination.cpp) | 80% | **Heavy** | Robust hashing and redirection implementation. |
| **Global Refining** | `global_refining.rs` | [GlobalRefining.cpp](../../../src/passes/GlobalRefining.cpp) | 60% | **Partial** | Functional LUB analysis; missing deep GC integration. |
| **Simplify Globals** | `simplify_globals.rs` | [SimplifyGlobals.cpp](../../../src/passes/SimplifyGlobals.cpp) | 20% | **Partial** | Basic removal logic; missing ~600 lines of rules. |
| **Signature Pruning** | `signature_pruning.rs` | [SignaturePruning.cpp](../../../src/passes/SignaturePruning.cpp) | 85% | **Complete*** | Covers unused/constant params; high parity with C++. |
| **Local Subtyping** | `local_subtyping.rs` | [LocalSubtyping.cpp](../../../src/passes/LocalSubtyping.cpp) | 70% | **Heavy** | Solid structural port; limited by `is_subtype` API. |
