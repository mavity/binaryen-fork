...
**Status**: 📋 Planned | **Passes**: 28 | **Estimated Effort**: 4–8 weeks | **Target**: Week 15-22
...
### Pass List
1. **inlining** — Basic function inlining
2. **inlining-optimizing** — Inlining + optimize at call sites
3. **inline-main** — Inline __original_main into main
4. **outlining** — Extract common code into functions (inverse of inlining)
5. **duplicate-function-elimination** — Merge identical functions
6. **merge-similar-functions** — Merge near-identical functions
7. **monomorphize** / **monomorphize-always** — Specialize polymorphic functions
8. **no-inline** / **no-full-inline** / **no-partial-inline** — Mark functions non-inlineable
...
### Pass List
1. **dfo** (dataflow-opts) — SSA-based optimizations
2. **flatten** (also in Tier 2B) — Convert to flat IR for SSA
3. **type-ssa** — Track types in SSA form
4. **gsi** (global-struct-inference) — Infer struct layouts
5. **cfp** / **cfp-reftest** (constant-field-propagation) — Propagate constant fields
6. **unsubtyping** — Remove unnecessary subtype relations
7. **local-subtyping** (also in Tier 2A/3C) — Refine via SSA
8. **gufa** — Global unreachable function analysis
...
3. Propagate copies (local.set $x (local.get $y) → replace $x with $y)
...
### Pass List
1. **global-refining** (also in 3C) — Narrow global types
2. **generate-global-effects** (also in 3B) — Compute whole-module effects
3. **gsi** (also in 4B) — Global struct inference
4. **gufa** (also in 4B) — Global unreachable function analysis
5. **gto** (global-type-optimization) — Optimize GC types globally
6. **remove-unused-module-elements** (also in Tier 1A) — Via global analysis
7. **simplify-globals** / **simplify-globals-optimizing** — Optimize globals
8. **propagate-globals-globally** — Propagate constants globally
...
### Pass List
1. **type-merging** — Merge compatible types
2. **minimize-rec-groups** — Split types into minimal recursion groups
3. **type-generalizing** — Generalize types for better compression
4. **signature-pruning** / **signature-refining** — Optimize function signatures
5. **heap-store-optimization** — Optimize struct/array stores
...
### Quantitative
- ✅ 28 passes implemented (48 → 76 total)
- ✅ Functional parity: ~75% of C++ capability
- ✅ Performance improvement: 10-50% from inlining
- ✅ Call graph + SSA infrastructure complete

### Qualitative
- ✅ Whole-module optimization pipeline functional
- ✅ Inlining delivers major performance wins
- ✅ SSA enables advanced data flow analysis
...
### Precipitous Summary
Tier 4 represents the bridge between a "functional" port and a "compiling" port. While the **infrastructure** (CallGraph, CFG, SSA) is largely present and functional, the **optimization logic** varies significantly between passes. Higher-level analysis like `SignaturePruning` and `DuplicateFunctionElimination` are near parity with C++, but core dataflow and inlining optimizations are still in early stages.

**Conclusion**: The "Bones" of Tier 4 are in place, but the "Muscle" (rulesets and heuristics) is missing for several advanced transforms, primarily due to the missing `is_subtype` and full GC type analysis in the core IR.

### Maturity Matrix

| Pass | Rust File | C++ File (Ref) | Maturity % | Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Inlining** | `inlining.rs` | [Inlining.cpp](../../../src/passes/Inlining.cpp) | 20% | **Partial** | Basic call inlining. Lacks production cost models/heuristics. |
| **DFO (DataFlowOpts)** | `dfo.rs` | [DataFlowOpts.cpp](../../../src/passes/DataFlowOpts.cpp) | 15% | **Skeletal** | SSA/CFG integration exists; no optimization logic (DCE/Prop). |
| **Flatten** | `flatten.rs` | [Flatten.cpp](../../../src/passes/Flatten.cpp) | 90% | **Complete*** | Robust implementation handling control flow, nesting, and side-effects. |
| **Duplicate Func Elim**| `duplicate_function_elimination.rs` | [DuplicateFunctionElimination.cpp](../../../src/passes/DuplicateFunctionElimination.cpp) | 85% | **Heavy** | Robust hashing, redirection, and unit tests. |
| **Global Refining** | `global_refining.rs` | [GlobalRefining.cpp](../../../src/passes/GlobalRefining.cpp) | 20% | **Partial** | Analysis structure present; missing real LUB/Subtyping logic. |
| **Simplify Globals** | `simplify_globals.rs` | [SimplifyGlobals.cpp](../../../src/passes/SimplifyGlobals.cpp) | 20% | **Partial** | Basic constant propagation; missing deep removal rules. |
| **Signature Pruning** | `signature_pruning.rs` | [SignaturePruning.cpp](../../../src/passes/SignaturePruning.cpp) | 90% | **Complete*** | Robust param pruning and call-site redirection; high parity. |
| **Local Subtyping** | `local_subtyping.rs` | [LocalSubtyping.cpp](../../../src/passes/LocalSubtyping.cpp) | 35% | **Partial** | Structural port exists; limited by missing `is_subtype` API. |
| **Type SSA** | N/A | [TypeSSA.cpp](../../../src/passes/TypeSSA.cpp) | 0% | **Missing** | Critical for GC optimization; not yet ported. |
...
The Tier 4 implementation currently lacks a robust **Type System API** for subtyping and Least Upper Bound (LUB) calculations, which cripples the refinement passes (`GlobalRefining`, `LocalSubtyping`). Furthermore, the **Inlining** pass needs a more sophisticated heuristic model (matching the C++ cost models) to avoid code-size blowup, and **DataFlowOpts (DFO)** requires full Def-Use chain integration to perform actual copy propagation and dead code elimination beyond simple placeholders. Finally, **GC-specific optimizations** (TypeSSA, GTO) are entirely missing and represent a significant part of Binaryen's modern value proposition.
