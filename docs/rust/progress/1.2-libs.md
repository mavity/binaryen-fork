...
- [ ] Implement arena allocation utilities (using `bumpalo` or `bumpalo`-backed crates)
 ...
 - [x] Add `hash` utilities and fast hashing with `ahash`
  ...
  - Add `ahash_bytes` helper in `rust/binaryen-support::hash` and `BinaryenAhashBytes` FFI helper in `rust/binaryen-ffi`.
  - Add `FastHashMap` FFI wrapper for `String -> uint64` via `BinaryenFastHashMap*` handlers and tests.
  - Add unit & smoke consumer tests to validate the map & helper; future work: expand FFI to arbitrary value types.

 - [x] Add benchmarks using `criterion` and property tests using `proptest`.
  - Create `bench/` or `benches/` in `rust/binaryen-support` to compare with C++ baselines (done; see `benches/bench_support.rs`).
  - Add CI benchmark job (nightly) to catch regressions.  (TODO: add CI job)

- [ ] Add code examples and API documentation in `docs/rust`.
 - [x] Add `cargo-fuzz` harnesses for `interner`, `arena`, and `ahash` in `fuzz/`; a `workflow_dispatch` sample exists at `.github/workflows/rust-fuzz.yml` and local `rust/scripts/run_cargo_fuzz.sh` is included.
  - Validation: local runs with `cargo-fuzz` (nightly) completed short 10s runs for each target without panics/crashes in this environment.
  - Note: `cargo-fuzz` requires the nightly toolchain (`rustup toolchain install nightly`) to run with sanitizer support; this repo includes a manual `workflow_dispatch` job for on-demand fuzzing.
