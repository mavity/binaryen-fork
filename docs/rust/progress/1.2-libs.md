...
- [ ] Add code examples and API documentation in `docs/rust`.
  - Status: Partial. The technical-specs and getting-started pages exist; crate-level `README.md` files and short C++ examples for FFI usage are still missing.
...
5. Cross-language tests for edge cases (Low-Medium priority)
  - Add extra tests in `test/rust_consumer` to verify threading, pointer re-use after `Dispose`, and expected failures on misuse.
