...
- [ ] Add code examples and API documentation in `docs/rust`.
 - [x] Add `cargo-fuzz` harnesses for `interner`, `arena`, and `ahash` in `fuzz/`; a `workflow_dispatch` sample exists at `.github/workflows/rust-fuzz.yml` and local `scripts/run_cargo_fuzz.sh` is included.
