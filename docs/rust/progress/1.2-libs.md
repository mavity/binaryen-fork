...
- Smoke & cross-language tests: `test/rust_consumer` + CI `ctest -R rust_ffi_smoke` entries exercised in `.github/workflows/rust-ci.yml`.
 - Smoke & cross-language tests: `test/rust_consumer` + CI `ctest -R rust_ffi_smoke` entries exercised in `.github/workflows/rust-ci.yml`.
 - Extra cross-language tests added: `test/rust_consumer/test_ffi_threaded.cpp`, `test/rust_consumer/test_ffi_arena_threads.cpp` (both `ctest` targets `rust_ffi_smoke_threaded`, `rust_ffi_smoke_arena_threads`). These are also covered by `test/rust_consumer/build_and_run.sh`.
